...
| `aliases`        | Where each kind of file is written, relative to the project root |
| `packageManager` | Which manager installs npm dependencies                          |

With the aliases above, `add button` writes `src/components/ui/button.tsx`
rather than `components/ui/button.tsx`. Only the leading segment is remapped —
