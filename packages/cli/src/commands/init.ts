...
export async function initCommand(projectName: string, options: InitOptions) {
  ...
  try {
    logger.info(
      `Creating Expo React Native project with UI library: ${projectName}`
    );
    ...
    // Create Expo project
    await createExpoProject(projectName, projectPath);
    ...
    await createExampleComponents(config);
...
async function createExpoProject(projectName: string, projectPath: string) {
...
async function createExampleComponents(config: any) {
  ...
  try {
    ...
    await createInputComponent(config);

    // Create Container component
    await createContainerComponent(config);

    // Create component index
    await createComponentIndex(config);
    ...
    logger.failSpinner('Failed to create example components');
    throw error;
  }
}

async function createButtonComponent(config: any) {
  const buttonPath = path.join(config.components, 'ui', 'button.tsx');
...
`;
  ...
  await fileOps.writeFile(buttonPath, buttonContent);
}

async function createInputComponent(config: any) {
  const inputPath = path.join(config.components, 'ui', 'input.tsx');
  const inputContent = `import React, { useState, forwardRef } from 'react';
...
export const Input = forwardRef<TextInput, InputProps>(({
  ...
  return (
    <View style={cn(styles.container, containerStyle)}>
      {label && (
        <Text style={cn(styles.label, { color: theme.colors.foreground })}>
          {label}
        </Text>
      )}
      <TextInput
      ...
      />
      {error && (
        <Text style={cn(styles.error, { color: theme.colors.destructive })}>
          {error}
        </Text>
      )}
      {helper && !error && (
        <Text style={cn(styles.helper, { color: theme.colors.mutedForeground })}>
          {helper}
        </Text>
      )}
    </View>
...
`;
  ...
  await fileOps.writeFile(inputPath, inputContent);
}

async function createContainerComponent(config: any) {
  const containerPath = path.join(config.components, 'layout', 'container.tsx');
...
`;
  ...
  await fileOps.writeFile(containerPath, containerContent);
}

async function createComponentIndex(config: any) {
  const uiIndexPath = path.join(config.components, 'ui', 'index.ts');
  const uiIndexContent = `export * from './button';
export * from './input';
`;

  await fileOps.writeFile(uiIndexPath, uiIndexContent);

  const layoutIndexPath = path.join(config.components, 'layout', 'index.ts');
  const layoutIndexContent = `export * from './container';
`;

  await fileOps.writeFile(layoutIndexPath, layoutIndexContent);

  const componentsIndexPath = path.join(config.components, 'index.ts');
  const componentsIndexContent = `export * from './ui';
export * from './layout';
`;

  await fileOps.writeFile(componentsIndexPath, componentsIndexContent);
}
...
export default function App() {
  const handleButtonPress = () => {
    Alert.alert('Button Pressed', 'Hello from Expo UI Library!');
  };

  return (
    <ThemeProvider>
      <Container style={styles.container}>
        <Text style={styles.title}>Expo UI Library</Text>
        <Text style={styles.subtitle}>Beautiful components for React Native</Text>
...
`;
  ...
  await fileOps.writeFile(appPath, appContent);
}
