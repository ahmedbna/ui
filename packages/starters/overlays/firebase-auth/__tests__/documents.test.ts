...
describe('taskFromDoc', () => {
  ...
  it('carries ownerId through', () => {
    // Every rule in firestore.rules keys on this, and the queries are required
    // to filter on it, so losing it in the mapper breaks reads and writes both.
    const result = taskFromDoc(
      snap('a', { text: 'x', isComplete: false, ownerId: 'uid-9' })
    );

    expect(result.ownerId).toBe('uid-9');
  });
});

describe('profileFromDoc', () => {
  /** A DocumentSnapshot, which unlike a QueryDocumentSnapshot may not exist. */
  const docSnap = (id: string, data: Record<string, unknown> | null) =>
    ({
      id,
      exists: () => data !== null,
      data: () => data,
  ...
  it('returns null when the document does not exist', () => {
    // Null means "not created yet", never "not onboarded". Nothing creates this
    // document but the provider itself, so between sign-up and that write there
    // is a real window — and treating it as onboarded: false would flash the
    // onboarding flow at a returning user.
    expect(profileFromDoc(docSnap('uid-1', null))).toBeNull();
  });

  it('reads a profile', () => {
    const result = profileFromDoc(
      docSnap('uid-1', {
  ...
  it('treats a missing onboarded flag as false, never as true', () => {
    // Defaulting the other way would skip onboarding for every user whose
    // document was written by an older version of the app.
    const result = profileFromDoc(docSnap('uid-1', { displayName: 'Ada' }));
